...
pub fn spawn_shutdown_handler(
    ...
    tokio::spawn(async move {
        ...
        // discord_api gRPC client retries against a different coord pod via
        // the mesh service. Setting this before anything else means we stop
        // taking on new in-flight work the moment SIGTERM arrives, even
        // while envoy's upstream-cache propagation lags.
        is_shutting_down.store(true, Ordering::Release);
        // Drain everything queued-but-undispatched. Sync nacks bubble up to
        // the sync RPC handler as Status::aborted, which discord_api can
        // retry on a different coord pod. Async nacks trigger immediate
